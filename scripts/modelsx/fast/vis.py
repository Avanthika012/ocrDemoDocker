...
def draw(img, boxes, words):
    ...
    for box in boxes:
        ...
        cv2.fillPoly(mask, [np.array(box)], color=(rand_b, rand_g, rand_r))
    
    img = cv2.addWeighted(img, 0.6, mask, 0.4, 0)
    
    for box, word in zip(boxes, words):
        if word == '###':
            cv2.drawContours(img, [np.array(box)], -1, (0, 0, 255), thickness[args.dataset])
        else:
            cv2.drawContours(img, [np.array(box)], -1, (0, 255, 0), thickness[args.dataset])
...
def visual(get_ann, data_dir, gt_dir, pred_dir, dataset):
    ...
    for index, (img_path, gt_path, pred_path) in tqdm(enumerate(zip(img_paths, gt_paths, pred_paths)), total=len(img_paths)):
        img = cv2.imread(img_path)  # load image using cv2
        ...
        img_pred = draw(img.copy(), pred, l)  # draw predictions on images
        img_gt = draw(img.copy(), gt, word)  # draw ground truths on images
        
        img_combined = np.hstack((img_gt, img_pred))  # stack two images horizontally
        
        mmcv.mkdir_or_exist(f"visual/{dataset}")
        output_path = f"visual/{dataset}/{img_path.split('/')[-1].split('.')[0]}_res.{img_path.split('.')[-1]}"
        cv2.imwrite(output_path, img_combined)  # save images using cv2
